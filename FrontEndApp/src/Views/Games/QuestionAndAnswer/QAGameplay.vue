    ...
    <div class="space-y-4">
        ...
        >
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="p-3 rounded-2xl bg-[var(--theme-bg-soft)] border border-[var(--theme-border)]">
                    <p class="text-2xl font-extrabold text-blue-600 dark:text-blue-400">{{ store.score }}<span class="text-base font-bold text-[var(--theme-text-soft)]">/{{ store.totalItems }}</span></p>
        ...
        <template v-else-if="question">
            <div class="p-5 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg-elevated)]">
                <p class="text-xs text-[var(--theme-text-soft)] mb-2 font-medium">Question {{ store.currentIndex + 1 }} of {{ store.totalItems }}</p>
                <p class="text-base font-medium text-[var(--theme-text)] leading-relaxed">{{ question.question }}</p>
            </div>
            ...
            <template v-if="store.gameState === 'answered'">
                <div v-if="question.proof || question.explanation" class="text-sm text-[var(--theme-text)] bg-[var(--theme-bg-soft)] p-3 rounded-xl border border-[var(--theme-border)] space-y-1">
                    <p v-if="question.proof" class="font-medium text-[var(--theme-text-soft)]">{{ formatProofReference(question.proof) }}</p>
                    <p v-if="question.explanation">{{ stripVerseHtml(question.explanation) }}</p>
                </div>
